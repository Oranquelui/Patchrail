...
## Next Milestones
1. Add the Phase 1 planning layer: split onboarding into `machine/runtime onboarding` and `project/planning onboarding`, and introduce the `Future Completion Brief -> Ontology Brief -> Product Brief -> Plan` sequence without changing the canonical state machine.
...
## Continuation Queue
1. Define the Phase 1 brief contract so `Future Completion Brief`, `Ontology Brief`, and `Product Brief` can be persisted as plan-scoped companion artifacts without inventing a second canonical record set.
2. Convert `patchrail start` from a simple splash shell into a two-pass onboarding entrypoint that first sets provider/access-mode/backend policy and then guides project planning.
